...
void hwpwm_init(const uint8_t pins[], uint8_t num_pins, uint pulse_freq_hz)
{
    ...
    if (pulse_freq_hz < 1907)
    {  // For lower frequencies, we can use the maximum wrap value of 65535 for best resolution, and adjust the divider to achieve the target
       // frequency
        wrap = 65535;
...
void servo_set_angle(uint gpio, float angle_deg)
{
    ...
    uint32_t pulse_us = SERVO_MIN_US + (uint32_t)((SERVO_MAX_US - SERVO_MIN_US) * (angle_deg / 180.0f));

...
void pca9685_init()
{
    ...
    printf("PCA9685 target %d Hz → prescale = %d (actual ~%.1f Hz)\n", SERVO_FREQ_HZ, prescale, 25000000.0f / (4096.0f * (prescale + 1)));
...
void pca9685_set_servo_angle(uint8_t channel, float angle_deg)
{
    ...
    uint16_t pulse_us = (uint16_t)(SERVO_MIN_PULSE_US + (SERVO_MAX_PULSE_US - SERVO_MIN_PULSE_US) * (angle_deg / 180.0f));
